...
TXT = {
    ...
    # ... (meer vertalingen volgen later in de code)
}
...
with preset_col1:
    if st.button("⭐ Typische as"):
        st.session_state["routing_editor"] = preset_dataframe("Typische as"); st.rerun()
with preset_col2:
    if st.button("⭐ Typische bracket"):
        st.session_state["routing_editor"] = preset_dataframe("Typische bracket"); st.rerun()
with preset_col3:
    if st.button("⭐ Typisch lasframe"):
        st.session_state["routing_editor"] = preset_dataframe("Typisch lasframe"); st.rerun()
with preset_col4:
    if st.button("⭐ Plaatwerk kast"):
        st.session_state["routing_editor"] = preset_dataframe("Plaatwerk kast"); st.rerun()

preset_col5, preset_col6 = st.columns(2)
with preset_col5:
    if st.button("⭐ Gietstuk behuizing"):
        st.session_state["routing_editor"] = preset_dataframe("Gietstuk behuizing"); st.rerun()
with preset_col6:
    if st.button("⭐ Gesmede flens"):
        st.session_state["routing_editor"] = preset_dataframe("Gesmede flens"); st.rerun()
...
bom_buy_cost_pc = 0.0 if bom_buy.empty else (
    bom_buy["UnitCost_eur"] * bom_buy["Qty_per_parent"] / (1 - bom_buy["Scrap_pct"].clip(upper=0.9))
).sum()
...
# Materiaalwaarde per stuk (inclusief waste + conversie) voor scrap/WIP berekening
...
# Energieprijsmix & learning curve
eff_kwh_price = (tou_day*price_day + tou_eve*price_eve + tou_night*price_night)
lc_fac = (max(Q,1)/max(lc_ref,1))**lc_b if lc_ref else 1.0

# =============================
# Routing-berekening (batches, parallel, WIP, CO₂ energie)
# =============================
def compute_costs_routing(routing_df):
    ...
    rows=[]; labor_total=machine_total=energy_total=qa_total=tooling_total=scrap_total=0.0
    ...
    for _, r in routing_df.iterrows():
        ...
        # Incrementele kostbasis voor volgende stap (voor WIP-schatting)
...
m4.metric("Shadow €/h (profit-only)", f"€ {shadow_profit_only:,.2f}/h")
m5.metric("CO₂ kg/stuk", f"{co2_per_part:,.2f} kg")

st.info(advice_text)

# =============================
# Kostensplitsing & grafieken
# =============================
split_df = pd.DataFrame({
    "Categorie":[
        "Materiaal (basis)","Conversie (gieten/smeden/extrusie)","Materiaalverlies/waste",
        "Inkoopdelen (BOM)","Arbeid","Machine","Energie","QA","Tooling",
        "Scrap-impact","Transport","Opslag","WIP/Flow holding","Rework","Overhead","Contingency","Profit"
    ],
    "Kosten (€)":[
...
if res_detail:
    ...
    st.expander("🔎 Detail per stap (€/stuk)", expanded=False).dataframe(det_df, use_container_width=True, hide_index=True)

# =============================
# Capaciteit & doorlooptijd
# =============================
st.subheader("🏭 Capaciteit & Doorlooptijd")
cap_df = pd.DataFrame(
...
with c_right:
    st.metric("Totale doorlooptijd (dagen)", f"{res['lead_time_days']:.2f}")
    st.metric("Bottleneck (proces)", bottleneck or "-")
    st.metric("Shadow €/h (gross margin)", f"€ {shadow_gross_margin:,.2f}/h")
  # =============================
...
def quick_eval(mat_key:str):
    m = materials[mat_key]
    # Gebruik prijs uit forecast alleen voor huidig gekozen materiaal; anders basisprijs
    ...
    # Snelle routing-evaluatie met materiaalspecifieke modifiers
    def eval_rout():
        if routing.empty:
            return 0, 0, 0, {}
        mat_k = m.get("k_cycle",1.0)
        tool_pc = m.get("tool_wear_eur_pc",0.0)
        lc = lc_fac  # zelfde learning-curve
        ...
        for _, r in routing.iterrows():
            ...
            eng_pc  = kwh*eff_kwh_price*qpp
            qa_pc   = (qa_min/60)*labor_rate*qpp
            tl_pc   = tool_pc*qpp
            ...
            hours=(run_min+setup_min)/60
            cap[r["Proces"]] = cap.get(r["Proces"],0.0)+hours

            # lead + wip
            proc_days = hours/max(hours_per_day,0.1)
            lead += qd + proc_days
            wip  += (inventory_cost_year/365.0) * (qd + proc_days/2.0) * cum_pc * Q
    ...
    sales = cost + prof
    co2pp = (gross * float(m.get("co2e_kgkg",0.0)))  # snelle proxy (materiaal-CO2)
    bottleneck = max(cap.items(), key=lambda kv: kv[1])[0] if cap else "-"
    return dict(Material=mat_key, Price_per_part=sales/max(Q,1), Lead_days=lead_days, CO2_per_part=co2pp, Bottleneck=bottleneck)
...
if mc_on:
    ...
    for i in range(mc_iter):
        # Materiaal & BOM met onzekerheid op materiaal
        ...
        sale_pp=(ct+pf)/max(Q,1); mc_vals.append(sale_pp)
    mc_vals=np.array(mc_vals)
    cc1,cc2,cc3=st.columns(3)
    cc1.metric("P50 €/stuk", f"€ {np.percentile(mc_vals,50):,.2f}")
    cc2.metric("P80 €/stuk", f"€ {np.percentile(mc_vals,80):,.2f}")
    cc3.metric("P90 €/stuk", f"€ {np.percentile(mc_vals,90):,.2f}")
...
def build_pdf(project, Q, material, sales_pp, sales_total, split_df, used_note, lang,
    ...
    y = H - 40
    c.setFont("Helvetica-Bold", 14); c.drawString(40,y, f"Maakindustrie Cost Tool+ – {project}"); y-=18
    ...
    c.drawString(40,y, f"Aantal: {Q}  |  Materiaal: {material}"); y-=14
    ...
    c.drawString(40,y, f"Verkoopprijs/stuk: € {sales_pp:,.2f}"); y-=16
    c.drawString(40,y, f"Totale verkoopprijs: € {sales_total:,.2f}"); y-=20
    ...
    c.setFont("Helvetica",10); c.drawString(50,y, advice_text[:95] + ("..." if len(advice_text)>95 else "")); y-=14
...
with exp_col1:
    ...
    st.download_button("📄 Download offerte (PDF)", data=pdf_buf,
